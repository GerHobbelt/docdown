...
var _ = require('lodash'),
    ...
    getHtmlCompliantHash = Entry.prototype.getHtmlCompliantHash;
...
function generateDoc(source, options) {
  ...
  _.each(api, function(entry) {
    ...
    var entryData = {
      ...
      'entryHref': '#${hash}',
      'entryLink': _.get(options, 'entryLink', '<a name="${hash}"></a>'),
  ...
  _.each(tocGroups, function(group) {
    var tocHref = 'toc-' + group.toLowerCase();
    tocMarkdown.push(
      '\n\n\n<!-- div -->\n',
      interpolate('## <a name="${tocHref}"></a> ${group}', {
        'tocHref': getHtmlCompliantHash(tocHref),
        'group': group
      }),
      ''
    );
  ...
  var tocHref = _.get(options, 'tocHref', '#' + getHtmlCompliantHash('toc-' + _.get(tocGroups, 0, '').toLowerCase()));
  if (tocHref) {
