...
function Entry(entry, source, lang) {
  ...
  // `this.getHtmlCompliantHash` is a *static* member method, hence doesn't need the `memoize` treatment.
 ...
 *
 * Note: Uses the `getHtmlCompliantHash()` API internally.
 *
 * @memberOf Entry
 * @param {string} [style] The hash style.
 * @returns {string} Returns the entry's hash value (without a hash itself), suitable for HTML ID/NAME and CSS usage
 * as per https://stackoverflow.com/questions/448981/which-characters-are-valid-in-css-class-names-selectors#answer-6732899
 * and https://www.w3.org/TR/2014/REC-html5-20141028/dom.html#the-id-attribute.
 */
function getHash(style) {
  ...
  if (style == 'github') {
    ...
    result += this.getCall();
    return getHtmlCompliantHash(result
      .replace(/[\\.=|'"(){}\[\]\t ]/g, '')
      .replace(/[#,]+/g, '-')
      .toLowerCase());
  }
...
/**
 * Turns a string into a hash value which is suitable for HTML ID/NAME and CSS usage
 * as per https://stackoverflow.com/questions/448981/which-characters-are-valid-in-css-class-names-selectors#answer-6732899
 * and https://www.w3.org/TR/2014/REC-html5-20141028/dom.html#the-id-attribute.
 *
 ...
 */
function getHtmlCompliantHash(str) {
  var result = _.toString(str);
  result = result
    .replace(/[. \s]/g, '-')
    .replace(/[^\w\d_:.-]/g, '_')   // rather strict HTML4 and HTML5 compliancy filter
    .replace(/-+/g, '-')
    .replace(/-$/, '')
    .replace(/^-/, '');
...
_.assign(Entry.prototype, {
  ...
  'getHash': getHash,
  'getHtmlCompliantHash': getHtmlCompliantHash,
